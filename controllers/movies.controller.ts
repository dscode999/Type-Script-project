...
export const createMovie = async (
  ...
  try {
    if (req.file) {
      ...
      const result = await uploadToCloudinary(file.buffer, "movies");
      (req.body as Record<string, unknown>).imageUrl = result.secure_url;
      (req.body as Record<string, unknown>).imagePublicId = result.public_id;
    }
